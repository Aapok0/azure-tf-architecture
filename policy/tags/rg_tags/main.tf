...
resource "azurerm_resource_group_policy_assignment" "rg_required_tags_pa" {
  ...
  description          = "Tag (${each.value["key"]}) required in all resources in resource group ${var.scope_name}"
  display_name         = "Tag (${each.value["key"]}) required in all resources in resource group ${var.scope_name}"
...
resource "azurerm_resource_group_policy_assignment" "rg_required_rg_tags_pa" {
  ...
  description          = "Tag (${each.value["key"]}) required in resource group ${var.scope_name}"
  display_name         = "Tag (${each.value["key"]}) required in resource group ${var.scope_name}"
...
resource "azurerm_resource_group_policy_assignment" "rg_inherited_rg_tags_pa" {
  ...
  description          = "Tag (${each.value["key"]}) that is inherited from resource group ${var.scope_name}"
  display_name         = "Tag (${each.value["key"]}) that is inherited from resource group ${var.scope_name}"
